...
async function inspectDirectory(directory, label) {
  ...
  else if (modeBits(metadata) !== 0o700) issues.push(`${label} permissions must be 0700`);
...
async function boundedNoFollowRead(file) {
  ...
  if (modeBits(metadata) !== 0o600) return { kind: 'invalid', issue: `${path.basename(file)} permissions must be 0600`, mode: modeBits(metadata) };
...
async function inspectMutationLock(paths) {
  ...
  if (metadata.size > 4096 || modeBits(metadata) !== 0o600) {
